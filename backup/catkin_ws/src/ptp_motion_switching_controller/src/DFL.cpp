...
double theta_kp=0.7,distance_kp=0.05,distance_kd=0.15; //theta_kp=0.6,distance_kp=0.02,distance_kd=0.15;
    ...
    if (first_step) {
        ...
        if(fabs(theta_error*180/3.14)<2){
           ...
           // ROS_INFO("linear vel output: %f, P:%f , D:%f",temp_distance_cmd,distance_error*distance_kp,distance_kd*belief.linear_velocity);
            if(temp_distance_cmd>0.09)
                temp_distance_cmd=0.09;
